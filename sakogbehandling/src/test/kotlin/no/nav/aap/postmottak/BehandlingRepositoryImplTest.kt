...
class BehandlingRepositoryImplTest {
    ...
    fun `hent behandling med id returnerer behandling`() {
        inContext {
            ...
            val hentetBehandling = behandlingRepository.hentMedLås(behandlingId)
    ...
    fun `hent behandling med journalpostId returnerer behandling`() {
        inContext {
            ...
            val hentetBehandling = behandlingRepository.hentMedLås(journalpostId)

            assertThat(hentetBehandling).isNotNull()
        }
    }

    private class Context(val behandlingRepository: BehandlingRepository)

    private fun <T> inContext(block: Context.() -> T): T {
        return InitTestDatabase.dataSource.transaction {
            val context = Context(BehandlingRepositoryImpl(it))
            context.let(block)
        }
    }
}